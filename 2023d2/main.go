...
func parse(s string) (Game, error) {
	g := Game{}
	ss := strings.Split(s, ": ")
	n, err := fmt.Sscanf(ss[0], "Game %d", &g.Id)
	if err != nil {
		return g, err
	}
	if n != 1 {
		return g, fmt.Errorf("only %d items successfully parsed", n)
	}
	for _, revealString := range strings.Split(ss[1], ";") {
		reveal := Reveal{}
		for _, diceCount := range strings.Split(revealString, ",") {
			var count int
			var colour string
			n, err := fmt.Sscanf(strings.TrimSpace(diceCount), "%d %s", &count, &colour)
			if err != nil {
				return g, err
			}
			if n != 2 {
